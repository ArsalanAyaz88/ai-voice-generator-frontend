...
export default function TextToSpeech() {
  ...
  // Clean up preview audio and cached URLs on unmount
  useEffect(() => {
    return () => {
  ...
  const handlePreview = async (voiceName: string) => {
    try {
      // If same voice is already playing, toggle pause/stop
      ...
      const cached = previewCacheRef.current[voiceName];
      if (cached) {
        const audio = new Audio(cached);
        previewAudioRef.current = audio;
        setPreviewPlaying(voiceName);
        audio.onended = () => setPreviewPlaying((cur) => (cur === voiceName ? null : cur));
        await audio.play();
        return;
      }
      ...
      previewCacheRef.current[voiceName] = url;
      ...
      // noop: preview errors should not block UI
    } finally {
  ...
  return (
    ...
    >
      ...
      >
          ...
          <button 
            onClick={() => router.push('/')}
            className="bg-zinc-800 text-white hover:bg-zinc-700 font-bold py-2 px-4 rounded-full text-lg transition duration-300"
          >
            &larr;
          </button>
                      ...
                      <li key={voice.name}>
                        ...
                        >
                          ...
                          <button
                            ...
                            } ${previewLoading === voice.name ? "opacity-60" : ""}`}
                            aria-label={`Preview ${formatVoiceLabel(voice)}`}
                          >
                            {previewLoading === voice.name ? "…" : previewPlaying === voice.name ? "⏸" : "▶"}
                          </button>
                        </div>
                      </li>
                    ))}
